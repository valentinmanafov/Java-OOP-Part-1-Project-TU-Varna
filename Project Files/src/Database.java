...
public class Database {
    ...
    public void addTable(Table table) throws DatabaseOperationException {
        ...
        if (tables.containsKey(tableName)) throw new DatabaseOperationException("Table named '" + tableName + "' already exists.");
        tables.put(tableName, table);
    }

    public Table getTable(String name) throws DatabaseOperationException {
        Table table = tables.get(name);
        if (table == null) throw new DatabaseOperationException("Table '" + name + "' not found.");
        return table;
    }

    public void removeTable(String name) throws DatabaseOperationException {
        if (tables.remove(name) == null) throw new DatabaseOperationException("Table '" + name + "' not found.");
    }
    ...
    public void renameTable(String oldName, String newName) throws DatabaseOperationException {
        if (oldName == null || newName == null || oldName.trim().isEmpty() || newName.trim().isEmpty()) throw new DatabaseOperationException("Table names cannot be null or empty.");
        if (oldName.equalsIgnoreCase(newName)) return;

        Table table = tables.get(oldName);
        if (table == null) throw new DatabaseOperationException("Table '" + oldName + "' not found.");
        if (tables.containsKey(newName)) throw new DatabaseOperationException("Table named '" + newName + "' already exists.");
        ...
        System.out.println("Table '" + oldName + "' successfully renamed to '" + newName + "'.");
    }
}